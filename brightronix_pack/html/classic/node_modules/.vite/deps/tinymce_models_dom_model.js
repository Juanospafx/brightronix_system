...
(function() {
  ...
  const detectBrowser$1 = (browsers2, userAgentData) => {
    return findMap(userAgentData.brands, (uaBrand) => {
      ...
      return find$1(browsers2, (browser) => lcBrand === browser.brand?.toLowerCase()).map((info) => ({
        current: info.name,
  ...
  const run = (operation, extract2, adjustment, postAction, genWrappers, table2, target, generators, behaviours) => {
    ...
    return output.bind((out) => {
      ...
      const tableSizing = Optional.from(behaviours?.sizing).getOrThunk(() => TableSize.getTableSize(table2));
  ...
  const elementFromGrid = (grid2, row2, column) => {
    const rows2 = extractGridDetails(grid2).rows;
    return Optional.from(rows2[row2]?.cells[column]?.element).filter(isEditable).orThunk(() => findEditableCursorPosition(rows2));
  };
