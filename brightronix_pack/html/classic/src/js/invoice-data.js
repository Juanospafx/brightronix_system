...
// src/js/invoice-data.js

// 1. Core Imports
import $ from 'jquery';

// 2. DataTables Imports
...
$(function() {
	/* --- DataTable Init --- */
	if ($("#invoice_datable_1").length > 0) {
        
		/* Checkbox Injection */
		let tdCnt = 0; // 'let' allows incrementing
		...
		var targetDt = $('#invoice_datable_1').DataTable({
			...
			"drawCallback": function () {
				$('#invoice_datable_1_wrapper').find('.pagination').addClass('custom-pagination pagination-simple justify-content-end');
			}
		});
        ...
        $("#invoice_datable_1").parent().addClass('table-responsive');

		/* Select All Logic */
		var DT1 = $('#invoice_datable_1').DataTable();
